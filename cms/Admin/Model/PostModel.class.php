...
class PostModel extends RelationModel{
	...
	const _PARAM_PICTURES = 'pictures';
	...
	**/
	public function getDetail($param){
		...
		//读取文章内容
		$postData = $PostM->relation(true)->where($map)->find();

		$post = $postData;

		$post[self::_PICTURES] = json_decode($postData[self::_PICTURES]); //反编译缩略图数组
	...
	**/
	...
	public function doUpdate($param){
		...
		//缩略图
		$data[self::_PICTURES] = json_encode($param[self::_PARAM_PICTURES]); //序列化数组
